...
export default function RenderProductsResults({ products }: Props) {
  ...
  if (device === "desktop") {
    return (
      <ul class="grid grid-cols-2 w-fit gap-y-[25px] gap-x-[57px]">
        {products
          ...
          groupedProducts.map((_, index) => (
            <li class="carousel-item">
              <Slider.Dot
                index={index}
                class={clx(
                  "bg-secondary h-2 w-2 no-animation rounded-full",
                  "disabled:w-[30px] disabled:bg-primary  transition-[background-color,_width]",
                )}
              >
              </Slider.Dot>
            </li>
