...
export default function MinicartFooter(props: Props) {
  const {
    cart: {
      storefront: { total, subtotal, coupon, locale, currency, enableCoupon = true, checkoutHref, sellerCode, cep, shippingValue },
    },
    state,
  } = props;
  ...
  useSignalEffect(() => {
    const minicartForm = document.getElementById(MINICART_FORM_ID) as HTMLFormElement;
    const postUrl = minicartForm.getAttribute("hx-post");
    if (!postUrl) return;
    const postURL = new URL(decodeURIComponent(postUrl), window.location.origin);
    const props = JSON.parse(postURL.searchParams.get("props") ?? "{}") as PropsSettings;
  ...
  return (
    <footer class="w-full">
      ...
      <div className="bg-secondary-content z-10 relative px-6">
        ...
        <Shipping cep={cep ?? ""} openShipping={openShipping} shippingValue={shippingValue ?? 0} />
        {/* Total */}
        ...
        <div class="flex gap-4 items-center py-5">
          ...
          <ButtonAnchor styleType={ButtonType.Primary} href={checkoutHref} className="h-11 w-1/2 minicartCheckout" hx-on:click={sendBeginCheckoutEvent}>
