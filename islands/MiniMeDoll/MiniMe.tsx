...
export default function MiniMe({ title, price, installments, image }: Props) {
  ...
  const fetchData = async () => {
    ...
    setData(data);
    const types = await LoaderMiniMeTypes();
    const typesSorted = [...types].sort((a, b) => a.ordem - b.ordem);
    ...
    <>
      <section class="container flex max-w-[1360px] w-full mb-[100px]">
        <section class="mr-[30px] px-[55px] py-[33px] bg-[#fff] border-dashed border-[1px] border-[#FF3800] rounded-[8px] max-w-[557px] h-[735px] w-full">
          <Image
            src={dollParts.length > 0 ? dollParts[dollParts.length - 1] : image}
            width={446}
            height={669}
          />
        </section>
        ...
        <div class="max-w-[773px] w-full h-[735px]">
          ...
          <div class="flex items-center relative mb-[36px] w-full">
            <p class="font-Quicksand bg-[#D6DE23] w-[40px] h-[40px] text-[20px] text-[#F98300] py-[4px] px-[14px] rounded-[50%] text-center font-bold">
              1
            </p>
            <hr
              class={count >= 0
            ...
            <div class="flex overflow-x-scroll max-w-[773px] h-[370px] w-full items-center">
              <div class="flex items-center">
                {filteredData.map((item) => (
                  <>
                    <div onClick={() => selectPart(item.id)} class="flex flex-col items-center w-[138px] h-[237] mr-[4px] bg-[#fff] rounded-[8px] border-[1px] border-[#D6DE23]">
